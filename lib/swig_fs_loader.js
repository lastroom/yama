 ...
 */
module.exports = function(overridings_path, basepath, encoding) {
    var ret = {};
    ...
    ret.resolve = function(to, from) {
        if (overridings_path) {
            var test = path.resolve(overridings_path, to);
            var exists = fs.existsSync(test);

            //console.log('trying with : ', test, ' => exits ?', exists);
        ...
        var ret = path.resolve(from, to);
        //console.log('2nd try with : from=', from, ', to=', to, ' => exits ?', ret);
