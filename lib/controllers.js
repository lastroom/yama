...
var renderTemplate = function(filename, args, res) {
    ...
    if (globals.templates) {
        template = swig.compileFile(globals.templates + '/' + filename + '.html');
    } else {
        template = swig.compileFile(__dirname + '/../templates/' + filename + '.html');
    }
...
controllers.setGlobals = function(opts) {
    ...
    models = require('./models.js')(globals.mongoose);
};
...
controllers.list = function(req, res) {
    ...
    if (req.cookies[globals.secret]) {
        var p = req.params.path,
            // YAMA field settings
            info = globals.info[p],
            Model = globals.mongoose.model(info.model);
        ...
        Model.list(options, function(err, results) {
            ...
            results.forEach(function(doc) {
                for (f in doc) {
                    if (info.fields[f] && typeof(info.fields[f].to_string) === 'function')
                        doc[f] = info.fields[f].to_string(doc[f]);
                }
            })
            ...
            Model.count().exec(function(err, count) {
                renderTemplate('list', {
                    title: helpers.capitalizeFirstLetter(info.label),
                    list: info.list || [],
                    fields: info.fields || [],
                    data: results,
                    path: p,
...
controllers.save = function(req, res) {
    ...
    if (req.cookies[globals.secret]) {
        ...
        Model.findOne({
            ...
            helpers.processFormFields(globals.info[p], req.body, function() {
                if (!id) {
...
controllers.edit = function(req, res) {
    ...
    if (req.cookies[globals.secret]) {
        var p = req.params.path,
            id = req.params.id,
            info = globals.info[p],
            Model = globals.mongoose.models[info.model];
        Model.load(id, function(err, doc) {
            ...
            helpers.processEditFields(info, doc, function() {
