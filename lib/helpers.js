    ...
    for (f in fields) {
        ...
        if (!isNew) {
            ...
            if (metaField.before_save && typeof(metaField.before_save) === 'function') {
                metaField.before_save(fieldName, body, Model, function(err, fname, b) {
                    if (err) {
                        console.log('Error in before_save of field %s', fname, err);
                        return cb(err);
                    }

                    count--;

                    //console.log(b[fname], '||======>', fname);
        ...
        // never called because isNew is never used.
            ...
            Model.findById(body[fieldName], function(err, ref) {
                if (err) console.error(err);
